...
//!
//! These tests were moved here from `src/oauth/middleware/tests.rs` by
...
//! The two remaining pure-function `require_scope` unit tests stay at
//! `src/oauth/middleware/tests.rs`, which needs no running instance.
//!
//! Real requests are driven through a minimal, test-only axum router via
//! `tower::ServiceExt::oneshot` — deliberately *not* the fully-wired
//! production router, which `tests/auth_scope_it.rs` covers separately over
//! a real socket. This file's own job is `BearerAuthMiddleware`'s extractors
//! against nothing but an `AuthState`. That router is backed by a real,
//! migrated Postgres schema via `kawasemi::test_harness::spawn_test_app`
...
use kawasemi::oauth::middleware::{AuthState, OptionalActor, RequiredActor, require_scope};
use kawasemi::oauth::model::ScopeSet as ModelScopeSet;
use kawasemi::oauth::scope::ScopeSet as RealScopeSet;
...
async fn register_test_app(pool: &sqlx::PgPool, runtime: &kawasemi::runtime::RuntimeContext) -> Id {
