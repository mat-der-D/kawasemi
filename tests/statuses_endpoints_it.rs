//! Integration tests for `kawasemi::statuses::endpoints`.
//!
//! Proves the router-level, auth/scope/response-code/`Link`-header behavior
//! that surface's own observable-completion condition names ("各エンドポイント
//! が正しい応答コード（200/401/403/404/422）とスコープ検証で動作し、ブック
//! マーク一覧に Link ヘッダが付く") — against a real, `spawn_test_app`-backed
//! Postgres schema, mirroring `tests/media_endpoints_it.rs`'s established
...
//! situation. `StatusService`/`InteractionService`/`PollService` are built
//! the same way `status_service`'s/`interaction_service`'s/`poll_service`'s
//! own unit tests already do (an in-memory `ActorHandleLookup`/
//! `LocalActorLookup`/`RelationshipQuery` plus a `RecordingSink` capturing
//! dispatched Activities) — the real `AccountService`/`LocalFsStore` this
//! crate's `AccountsModule`/`MediaModule` already build for `TestApp` are
//! reused unchanged for account/media rendering, so `StatusRenderInput`'s
//! `account`/`media_attachments` fields are genuinely resolved through
//! already-reviewed code, not faked.
//!
//! The pure, DB/network-free wire-shape helpers
//! (`parse_visibility`/`parse_media_ids`/`parse_optional_limit`/`parse_id`)
//! stay in `src/statuses/endpoints/tests.rs`, where they can reach those
...
use kawasemi::actor::{ActorState, ActorType, Handle, NewActor, ResolvedActor};
use kawasemi::config::Secret;
use kawasemi::domain::{Id, Visibility};
use kawasemi::error::AppError;
use kawasemi::federation::outbound::target::{DeliveryTarget, RecipientTargetResolver};
use kawasemi::federation::{
    ActorUrls, CanonicalActivity, DeliveryService, DeliverySink, LocalActorLookup,
};
...
use kawasemi::runtime::{IdGenerator, RuntimeContext, SeqIdGenerator};
use kawasemi::statuses::activity_builder::{ActorHandleLookup, StatusActivityBuilder};
use kawasemi::statuses::endpoints::{
    BOOKMARKS_PATH, POLL_PATH, POLL_VOTES_PATH, STATUS_BOOKMARK_PATH, STATUS_CONTEXT_PATH,
    ...
    STATUS_UNREBLOG_PATH, STATUSES_PATH, StatusesEndpointsState, bookmark_status, create_status,
    delete_status, edit_status, favourite_status, list_bookmarks, pin_status, reblog_status,
    show_poll, show_status, status_context, status_history, status_source, unbookmark_status,
    unfavourite_status, unpin_status, unreblog_status, vote_poll,
};
use kawasemi::statuses::interaction_service::InteractionService;
use kawasemi::statuses::poll_service::PollService;
use kawasemi::statuses::status_service::{MentionLookup, StatusService};
use kawasemi::statuses::visibility::{RelationshipQuery, ViewerRelation};
use kawasemi::test_harness::query_log::{QueryKind, record_queries};
...
/// The migrated stand-in for this file's original
/// `impl DeliverySink for Arc<RecordingSink>`: an integration-test crate may
/// not implement the foreign `DeliverySink` trait for the foreign generic
/// application `Arc<RecordingSink>` (E0117 — `Arc` is not `#[fundamental]`),
/// so the shared handle is wrapped in a local newtype instead. `Clone` is the
/// `Arc`'s clone, so every clone keeps recording into the one
/// [`RecordingSink`] `build_state` hands back.
#[derive(Clone)]
...
struct MockRelationshipQuery {
    is_follower: bool,
    followers: Vec<kawasemi::federation::Recipient>,
}

impl MockRelationshipQuery {
    fn new(is_follower: bool, followers: Vec<kawasemi::federation::Recipient>) -> Self {
        Self {
            is_follower,
            followers,
        }
...
impl RelationshipQuery for MockRelationshipQuery {
    ...
    async fn followers_of(
        &self,
        _author: Id,
    ) -> Result<Vec<kawasemi::federation::Recipient>, AppError> {
...
fn build_state(
    ...
    let followers = known_actors
        .first()
        .map(|(_, handle)| {
            vec![kawasemi::federation::Recipient::Local(
    ...
    let status_service = Arc::new(StatusService::new(
        ...
        StatusActivityBuilder::new(
            ...
            Arc::new(DeliveryService::new(
                ...
                SharedRecordingSink(Arc::clone(&local_sink)),
                SharedRecordingSink(Arc::clone(&http_sink)),
            )),
        ),
        relationship.clone(),
        actor_lookup.clone(),
        kawasemi::statuses::notification_sink::NotificationSinkRegistry::new(),
    ));
...
async fn poll_vote_updates_the_tally_and_get_reflects_it() {
    ...
    let status = kawasemi::statuses::model::Status {
        id: status_id,
        actor_id,
        uri: uri.clone(),
        url: Some(uri),
    ...
    kawasemi::statuses::status_repository::insert_status(&app.pool, &status)
        .await
        .expect("insert_status must succeed");
    kawasemi::statuses::poll_repository::insert_poll(
        &app.pool,
        &kawasemi::statuses::model::Poll {
            id: poll_id,
            status_id,
            expires_at: None,
            multiple: false,
        ...
        &[
            kawasemi::statuses::model::PollOption {
                poll_id,
                idx: 0,
...
async fn attach_tag(app: &TestApp, status_id: Id, name: &str) {
    let tag = kawasemi::statuses::model::Tag {
        id: app.runtime.ids.next_id(),
        name: name.to_string(),
        created_at: app.runtime.clock.now(),
    };
    let tag = kawasemi::statuses::tag_repository::upsert_tag(&app.pool, &tag)
        .await
        .expect("upsert_tag must succeed");
    kawasemi::statuses::tag_repository::associate_tag(&app.pool, status_id, tag.id)
        .await
...
async fn status_context_renders_every_ancestor_and_descendant_material_in_thread_order() {
    ...
    .await;
    kawasemi::statuses::interaction_repository::add_favourite(
        &app.pool,
        viewer,
        visible_target,
...
async fn bookmark_list_renders_every_material_newest_bookmark_first() {
    ...
    for status_id in [first, second, polled, boost_of_visible, boost_of_hidden] {
        kawasemi::statuses::interaction_repository::add_bookmark(
            &app.pool,
