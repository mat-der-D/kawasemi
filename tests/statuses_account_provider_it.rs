//! What one whole [`AccountStatusesProviderImpl::list_statuses`] page
//! renders, and what it costs in queries — the part of
//! `src/statuses/account_provider/tests.rs` that needs a live
//! `spawn_test_app` instance and therefore belongs here rather than at the
//! unit-test position. The strict `RequiredPolls` resolver tests stay in
//! that module: they run on `spawn_test_db` and reach a private type.
//!
//! Scoped to the provider's own assembly rather than to the HTTP surface:
//! `tests/account_statuses_provider_it.rs` drives the real router and
//! proves the port is wired at all, while this file calls
//! `AccountStatusesProviderImpl` directly so that what a page is *made of*
//! — and how many queries making it costs — can be observed without a
//! request in the way.
//!
//! Row shapes mirror `statuses/poll_repository/tests.rs`'s fixture
//! convention: a poll needs a real `statuses` row to reference
//! (`polls.status_id` is a genuine FK), but `statuses.actor_id` is not
//! FK-constrained, so a bare id stands in for the author wherever no
//! rendered Account is needed.

use serde_json::Value;

use kawasemi::accounts::ports::{AccountStatusesProvider, StatusesQuery};
...
use kawasemi::media::media_repository::insert_media;
use kawasemi::media::model::{Focus, Media, MediaState, MediaType};
use kawasemi::media::store::ObjectKey;
use kawasemi::runtime::RuntimeContext;
use kawasemi::statuses::account_provider::AccountStatusesProviderImpl;
use kawasemi::statuses::model::{Poll, PollOption, Status, Tag};
use kawasemi::statuses::tag_repository::{associate_tag, upsert_tag};
use kawasemi::statuses::{interaction_repository, poll_repository, status_repository};
...
/// same helper `tests/search_hydrator_it.rs` and `render_assembler/tests.rs`
