//! Integration tests for `RequestSigner` (Requirements 1.1, 1.2, 1.3, 1.5 of
//! the federation-core spec), relocated here from
//! `src/federation/signatures/signer/tests.rs` by
//! `.kiro/specs/test-placement-migration` task 2.1: every test below needs a
//! real, running instance (`spawn_test_app`), which steering
//! `structure.md`'s test layout rule places under `tests/*_it.rs`.
//!
//! Follows `src/actor/directory/tests.rs`'s and
...
//! exists but has no key" path) rather than mocks. `app.runtime.keys` is
//! the real, DB-backed `DbSigningKeyProvider`, so these tests exercise the
//! exact same `SigningKeyProvider` wiring the running application would use,
//! not a fixed stand-in.
//!
...
//!
//! The pure unit tests for `http_date` / `host_from_url` need no running
//! instance and stay at `src/federation/signatures/signer/tests.rs`.
...
use kawasemi::actor::model::{ActorState, ActorType, LocalActor};
...
use kawasemi::federation::signatures::{
    Digest as BodyDigest, DraftCavageSuite, OutboundRequest, RequestSigner, Rfc9421Suite,
    SignableRequest, SignatureFormat, SignatureSuite,
};
...
/// `id-sha256`). Test-local copy of `signer.rs`'s own private constant: it is
/// used here only to *build the expected verification input*, never as the
/// subject under test, so duplicating it changes nothing these tests assert
/// (the migration must not widen the production module's public surface --
/// `.kiro/specs/test-placement-migration/requirements.md` 3.1).
...
/// SHA-256. Test-local copy of `signer.rs`'s own private helper -- see
/// [`SHA256_PKCS1V15_PREFIX`] for why it is duplicated rather than exported.
