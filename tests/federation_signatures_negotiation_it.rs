//! Integration tests for `SignatureNegotiator` (Requirements 3.1, 3.2, 3.3
//! of the federation-core spec), relocated here from
//! `src/federation/signatures/negotiation/tests.rs` by
...
//!
//! Follows `tests/federation_signatures_signer_it.rs`'s and
...
//! `MockFederationHttpClient` so the "which format was sent, in what order,
//! how many times" assertions are fully deterministic without any real
//! network call.
//!
//! The pure unit tests for `format_to_db` / `format_from_db` /
//! `other_format` need no running instance and stay at
...
use kawasemi::federation::signatures::{
    HttpResponse, MockFederationHttpClient, OutboundRequest, RequestSigner, SignatureNegotiator,
};
