//! Integration tests for [`kawasemi::search::hydrator::SearchHydrator`]
//! (task 4.2 completion definition: "重複アカウントが一意化され、閲覧者不可視
//! の投稿が必ず除外され、ハッシュタグが Tag JSON 化される統合テストが通る";
//! Requirements 1.2, 3.2, 3.3, 3.5, 4.2, 4.6, 5.2).
//!
//! Every test below is a real, executable DB-backed integration test against
//! `kawasemi::test_harness::spawn_test_app`. These tests were moved here from
//! `src/search/hydrator/tests.rs` by `.kiro/specs/test-placement-migration`
//! task 3.2, so that steering `structure.md`'s test layout rule ("DB込みの
//! 実起動インスタンスを要する検証は `tests/` 直下の `*_it.rs` に置く") holds
//! in fact and not only on paper. `src/search/hydrator/tests.rs` keeps the
//! tests that need no running instance: the pure-logic `account_ref_id`
//! test, and the three `TolerantPolls::resolve_many` tests, which construct
//! that module-private type directly and therefore cannot be expressed from
//! outside the crate without promoting it (Requirement 3.1 forbids that).
//!
...
//! because `TolerantPolls`'s own fixtures left behind still use them;
//! `seed_custom_emoji` mirrors `statuses/render_assembler/tests.rs`'s
//! identical test-local helper). `account_ref_id` is redefined test-locally
//! below for the same reason: it is a one-line private helper of
//! `kawasemi::search::hydrator`, and this file needs its value, not its
//! implementation (Requirement 3.1 — no production item is promoted).
...
use kawasemi::accounts::model::RelationshipView;
use kawasemi::accounts::ports::RelationshipStateProvider;
...
use kawasemi::statuses::interaction_repository::add_favourite;
...
// -- test-local redefinition of `search::hydrator`'s own one-line private
// helper (see this file's doc comment) --------------------------------------
...
/// `kawasemi::accounts::ports::tests::FixedRelationshipProvider`'s identical
...
/// `kawasemi::statuses::visibility::is_visible`'s own author-always-visible
