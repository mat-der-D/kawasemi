...
- [ ] 1.2 マイグレーションディレクトリと初期マイグレーションを用意する
...
- [ ] 5.2 (P) IdGenerator 境界を実装する
  - `IdGenerator` trait と本番実装・決定的実装（シード由来の連番）を実装する
...
- [ ] 5.4 (P) SigningKeyProvider 境界を実装する
  - `SigningKeyProvider` trait とテスト用固定鍵実装を実装し、本番実装を actor-model が差し込む拡張点を残す
  - 単体テストでテスト用実装が固定鍵を再現的に返すことを確認できる
  - _Requirements: 5.4_
  - _Boundary: RuntimeContext/signing_key_

- [ ] 5.5 RuntimeContext 集約を実装する
...
- [ ] 5.6 (P) 共有ドメインプリミティブの正準定義を実装する
  - `src/domain/primitives.rs` に、並行 spec 横断で共有される軽量プリミティブを唯一の正準定義として実装し、`src/domain/mod.rs` から再公開する
  - `AccountRef`（`Local(Id)`/`Remote(Id)`、core-runtime の `Id` 型を使用）を、所有者情報を露出せず Account エンティティの知識を持たない純粋プリミティブとして定義する
  - `Visibility`（`Public`/`Unlisted`/`Private`/`Direct`）の列挙と serde/文字列表現の対応付けのみを定義し、公開範囲ポリシー（振る舞い）は含めない（statuses-core が所有）
  - 単体テストで「`Visibility` の serde/文字列表現が各バリアントで安定」「`AccountRef` がローカル/リモートを区別する」ことを確認できる
...
- [ ] 7.4 Bootstrap composition root を実装する
  ...
  - _Boundary: Bootstrap_
  - _Depends: 2.1, 3.1, 4.1, 4.2, 5.5, 7.3_
...
- [ ] 8.1 DB 込み統合テストハーネスを実装する
  ...
  - テスト完了時に分離 DB・listener・プールを解放する（`Drop` 等）
  - 統合テストハーネス自体を用いた最小テストが起動・終了でき、適用済み DB と決定的注入が得られることを確認できる
...
- [ ] 9.1 ライフサイクルとマイグレーション安全性の統合テストを追加する
  - `spawn_test_app` による起動でマイグレーション適用済み DB が得られヘルス確認が応答すること、shutdown でリソースが解放されることを検証する
