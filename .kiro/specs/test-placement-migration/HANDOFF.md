...
| フルスイート実行時間（中央値） | 634.80 秒 | 640.44 秒（比 **1.01**） |
...
- **要件 5.5（隔離スキーマ）** — 実行による新規生成 0。判定は総数ではなく集合差で行った（後述 C-9）。
  最終ツリーでの 3 試行すべてで独立に 0 を確認（`timing.md` §8）
- **要件 5.2（実行時間）** — 最終ツリーで測り直した中央値 640.44 秒、比 1.01（`timing.md` §8）。
  当初の計測は `cleanup()` 追加前のツリーで行われており、検証で齟齬が検出されたため再計測した
