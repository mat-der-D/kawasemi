...
### 付記 3. 要件 5.5 の判定基準（**解消済み — 読み替えは不要になった**）

**［以下 3 段落は解消前の記述である。結末は本節末尾の【解消】を参照］**
...
#### 【解消】読み替えは不要になった（タスク 8.3 / 8.4）

**上の暫定提案は採用しない。要件 5.5 は元の文言のまま、絶対値 0 で充足している。**
経緯を残す意味で上の記述は削除せず、ここに結末を追記する。

1. **前提が崩れた。** 上の議論は「フルスイート 1 回につき 1 個残るのは現行ハーネスでは
   原理的に不可避」という移設前ベースラインに立脚していた。しかし**タスク 8.3 が真因を特定して除去した** —
   残留の実体はリーパーの構造ではなく、**`cleanup().await` を呼んでいなかった 4 ファイル**だった
   （`timing.md` §7.3 / §7.4）。4 ファイルに teardown を加えて残留 4 → 0 になり、
   **`--test-threads=1` でも 0** であることが、頻度低減ではなく真因除去であることを示している。
2. **スイート全体で 0 が確定した。** タスク 8.4 のフルスイート実測（`verification.md` §8.2）で、
   **本実行が新たに残した隔離スキーマは 0 件**（実行前後のスキーマ**集合差**で判定。
   総数 24 → 20 の減少は起動時スイープが過去の置き土産 4 件を回収したもので本実行の産物ではない）。
3. したがって**判定基準の読み替えも、要件 5.5 の改訂も、人間承認も必要ない。**
   本節が求めていた承認ゲートはここで閉じる。

**ただし構造的リスクは残る。** 真因のさらに下にある「リーパーのキューがプロセス終了時に
無条件破棄される」（`src/test_harness/reaper.rs:112-113`）は先行 spec の所有物で本 spec の
Out of Boundary にあたり未修正である。`cleanup()` を呼ばないテストが今後追加されれば同じ残留が
再発し、**それを検出する仕組みはリポジトリに存在しない**（`timing.md` §7.6）。
これは例外台帳の項目ではなく `HANDOFF.md` の引き継ぎ事項である。
...
- 上記コマンドはすべて **`git grep`** であり、走査対象は**追跡ファイルの作業ツリー**である
  （リビジョンを引数に取る形だけがそのリビジョンの木を走査する）。したがって未コミットの
  変更は計数に反映され、**未追跡ファイルは数えない**。再現時はワークツリーの状態を先に確認すること。
  （本行は当初「インデックスを走査」と誤記しており、`verification.md` の記述と矛盾していた。
  タスク 8.4 のレビュー round 1 で訂正。数値への影響はない。）
