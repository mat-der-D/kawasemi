...
- ブロック判定の委譲境界（`BlockPolicy`、既定 no-op）。
...
- `BlockPolicy` の問い合わせ契約の変更。
- 署名スイート（`SignatureSuite`）・`PublicKeyResolver` / `FederationHttpClient` 境界のシグネチャ変更。
- アクター URL・inbox/outbox URL の構築規約（URL 形）の変更。
- 連合用テーブルのスキーマ・配送ジョブ状態モデル・重複排除キーの変更。
...
└── federation/
    ...
    ├── config.rs                 # 連合関連設定の参照（サーバードメイン・セキュアモード・配送リトライ方針）
    ...
    └── endpoints/
        ...
        ├── ap_get.rs             # アクター/オブジェクト/コレクション activity+json GET・authorized fetch・content negotiation
        ├── inbox.rs              # inbox / shared inbox POST ハンドラ
        ├── outbox.rs             # outbox GET（順序付きコレクション・ページング）
        └── document.rs           # ActivityPubDocumentBuilder: アクター/コレクション ActivityPub 表現構築
...
署名検証→必須プロパティ検証→ブロック判定→重複排除→ディスパッチの順。各拒否は意味論処理前に行う（2.6, 7.2, 9.3, 12.2）。重複は再処理せず受領応答する（7.4）。
...
| 6.1–6.6 | アクター/オブジェクト/コレクション GET・content negotiation・authorized fetch・owner 非露出・未検出 | ApGet handler, ActivityPubDocumentBuilder, ActorUrls, SignatureVerifier | build_actor_document() | 受信パイプライン（authorized fetch） |
...
| 8.1–8.3 | outbox コレクション・ページング・範囲外除外 | Outbox handler, ActivityPubDocumentBuilder | build_outbox_page() | （ハンドラ） |
...
| ActivityPubDocumentBuilder | Endpoint | アクター/コレクション AP 表現構築 | 6,8 | ActorUrls, JsonLdCodec, ActorDirectory (P0) | Service |
...
依存方向（左→右、上位は下位のみ参照）: `JsonLdCodec / ActorUrls / SignatureSuite / Digest / FederationHttpClient → RequestSigner / SignatureVerifier / PublicKeyResolver / ReceivedActivityStore / BlockPolicy / DeliveryQueue → SignatureNegotiator / InboundActivityDispatcher / RecipientTargetResolver / ActivityPubDocumentBuilder → InboxService / DeliverySink → DeliveryService / DeliveryWorker → Endpoints → FederationModule wiring`。
...
pub trait SignatureVerifier: Send + Sync {
    fn verify_request(&self, req: &IncomingRequest) -> Result<VerifiedSigner, AppError>;
}
```
...
**Responsibilities & Constraints**
- `PublicKeyResolver` は keyId→公開鍵を解決し、`remote_public_keys` にキャッシュ（2.3）。有効キャッシュ時はネットワーク取得しない（2.4）。
...
pub trait BlockPolicy: Send + Sync {
    async fn is_blocked(&self, actor_uri: &str) -> Result<bool, AppError>;
}
```
...
- ローカル in-process 配送からも署名検証を除いた同一意味論経路を提供（10.3 を満たすための共通処理 `process_local`）。
...
##### Service Interface
```rust
/// リモート受信: 署名検証込みのフルパイプライン。
pub async fn process_inbound(&self, req: IncomingRequest) -> Result<InboxOutcome, AppError>;
/// ローカル in-process: 署名検証を除く同一意味論経路（重複排除・ディスパッチ）。
pub async fn process_local(&self, activity: ParsedActivity, signer: VerifiedSigner) -> Result<InboxOutcome, AppError>;
```
- Invariants: `process_inbound`（署名検証後）と `process_local` は同一の重複排除・ディスパッチ処理に合流する（10.3, 10.5 の構造的担保）。
...
// LocalDeliverySink → InboxService::process_local
...
- AP GET: content negotiation（9.4, 6.3）、セキュアモード時 authorized fetch 検証（6.4）、未検出（6.6）。
- inbox/shared inbox: `InboxService::process_inbound` 呼び出し、検証失敗は認証失敗応答（7.2）。
- outbox: `build_outbox_page` を返す（8.x）。
...
- 下流提供境界: `InboundActivityHandler`（受信委譲）・`DeliveryRequest`（配送依頼）・`BlockPolicy`（ブロック委譲）。
...
- 設定: セキュアモードフラグ・配送リトライ方針を core-runtime 起動設定に追加。
