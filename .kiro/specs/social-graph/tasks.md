...
- タスク 6.3: `spawn_federation_pair`（federation-core、`src/federation/test_harness.rs::spawn_paired_instance`）は task 5.2（コミット `ce33f3a`）の時点で既に `social_graph::register_downstream_handlers` を両インスタンスへ配線済みであることを実装読解で確認し（`statuses-core` task 8.3 とは異なりハーネス拡張は不要）、ハーネス自体には手を入れず `tests/federation_symmetry_it.rs` のみを追加した。Requirement 10.3 の核心（同一 Activity・同一サービス呼び出しでローカル宛/リモート宛の関係状態遷移結果が同値）は、同一 `FollowService::follow`/`BlockService::block` 呼び出しをターゲットの locality だけ変えて実行し、双方の関係状態フィールド（`follow.{reblogs,notify,languages}`/`followed_by`/`requested_by`/`blocked_by`）を `assert_eq!` で比較する形で検証した（各インスタンス自身の DB から読み取り、送信元視点だけで済ませていない）。ブロック拒否の 2 インスタンス版は `delivery_jobs.attempts >= 1` かつ `received_activities` 不在の組み合わせで検出しており、単一インスタンス版 `block_policy_it.rs`（task 6.2）の直接 403 アサーションよりは間接的だが、`worker.rs`/`queue.rs` の `mark_done`/`reschedule` 実装上、成功時（2xx）は `attempts` を増やさないため BlockPolicy が退行すればこのテストはタイムアウト/失敗する（サイレントパスにはならない）ことをレビューで確認済み。これで社会グラフ spec の全 11 タスク（グループ 1-6）が完了。
