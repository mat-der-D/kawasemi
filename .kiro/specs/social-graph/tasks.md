...
- タスク 6（feature-level `kiro-validate-impl` remediation）: グループ 6 完了により social-graph 全 26 サブタスクが完了した時点で走った feature-level `kiro-validate-impl` が、design.md の Boundary Commitments（39行目）にのみ記載され、どのタスクの `_Boundary:_` にも一度も現れていなかった約束——本 spec が statuses-core の `RelationshipQuery` 委譲ポート（`viewer_relation()` / `followers_of()`、`src/statuses/visibility.rs`）へ本実装を供給する——が実際には一度も配線されていなかったことを検出した。`NoRelationshipQuery`（常に「関係なし・フォロワー空」の安全側既定）が本番で恒久的に使われ続けており、フォロワー限定可視性（`VisibilityPolicy::is_visible`）とフォロワー宛アドレッシング（`Addressing::derive_recipients`）が、social-graph が完全実装済みにもかかわらず、全アカウントに対して常に「フォロワーなし」として振る舞っていた（`GET /api/v1/accounts/:id/statuses` を含む）。task 5.2 が federation-core の `BlockPolicy` に対して確立した先例（`BlockPolicyRegistry`：`Arc<RwLock<Arc<dyn Trait>>>` + `&self` `set_*`、既定動作を変えない実行時差し替え）と同型の `RelationshipQueryRegistry` を `src/statuses/visibility.rs` に新設し、`src/social_graph/providers.rs` に `RelationshipQueryImpl`（`RelationshipRepository` の既存クエリ——`load_states`・新設 `followers_of`——を再利用）を実装して登録した。初回のリモディエーション実装は `StatusService`/`InteractionService`/`PollService` 経路のみを配線し、`AccountStatusesProviderImpl::visible_to`（`GET /api/v1/accounts/:id/statuses` を裏で支える、`R` ジェネリックを取らない構造的に別の呼び出し経路）が `NoRelationshipQuery` を直接ハードコードしたまま見落とされていたが、レビューで指摘され `register_account_ports` にも同じレジストリを追加配線して解消した（コミット `79c2e70`）。新規テスト `tests/social_graph_visibility_query_it.rs`（3件）が `GET /api/v1/statuses/:id` と `GET /api/v1/accounts/:id/statuses` の両経路で実フォロー状態が反映されることを検証する。今後 statuses-core 側で `RelationshipQuery` を消費する新しい経路を追加する場合は、必ず `RelationshipQueryRegistry` 経由で取得すること（`NoRelationshipQuery` を直接構築しない）。
...
- タスク 6.1: `tests/follow_unfollow_it.rs`（10件）/`follow_request_it.rs`（6件）/`same_server_skip_it.rs`（3件）/`mute_block_it.rs`（12件）を追加し `spawn_test_app` 経由の実 HTTP で Requirement 1,2,3,4,5 の主要シナリオを検証（本タスクは検証のみで `src/` は無変更）。レビューで発見した既存の未修正バグ（本タスクの境界外、将来タスクでの修正を推奨）: **ローカル宛 Block はレビューで確認済みの `activity_id` 上書きバグを持つ** — `BlockService::block` が実 `activity_id` で `Transitions::apply_block` を実行した後、`DeliveryService` がローカル宛配送を in-process 同期処理として `SocialGraphInboundHandler::handle_block` へ再合流させ、そこから `Transitions::mark_blocked_by`→`block_and_clear(blocker, blocked, activity_id=String::new())` が呼ばれ、`repository::upsert_block` の `ON CONFLICT ... DO UPDATE SET activity_id = EXCLUDED.activity_id` が直前に永続化した実 `activity_id` を空文字列で上書きする。Requirement 5.5（ローカル宛 Block/Undo の対称配送）は本タスクの `_Requirements:_` に含まれないため未検証のまま据え置いたが、修正は `transitions.rs`/`inbound.rs`（本タスクの `_Boundary:_` 外）が必要。今後この経路（ローカル宛 Block の Undo）を扱うタスク（6.3 の連合対称性テストや将来のフォローアップ）はこの既知バグを踏まえること。
