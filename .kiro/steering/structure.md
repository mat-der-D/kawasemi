...
### テストを `tests.rs` から `tests/*_it.rs` へ移すとき

- **移設でテスト関数が 0 本になった `tests.rs` は削除し、親の `#[cfg(test)] mod tests;` 宣言も外す。** 移設先へのポインタは**本番モジュール自身の `//!` doc** に畳み込む。`#[cfg(test)]` 配下の doc は `cargo doc` に描画されないため、そこに残しても案内板として機能しない。worked example: `src/search.rs` / `src/accounts/account_service.rs`。「`tests.rs` があるなら単体テストがある」という不変条件も同時に保てる。
- **部分移設で `tests.rs` が残る場合も、親 `.rs` の doc が偽になっていないかを同じ作業の中で確認して直す。** 参照先ファイルはパスとして有効なまま中身だけが移設先へ移るので、**ファイルの実在ではなく名指しされた項目の実在**で検算する。後からまとめて掃除しようとすると、パス修飾・モジュール修飾・裸の 3 形すべてに針を張る必要が生じて必ず漏れる。
- **doc コメントに `spawn_test_app` を書くときは開き括弧を付けない**（`` `spawn_test_app` `` と書く）。配置規約の充足を数える `git grep 'spawn_test_app('` が doc の記述を拾い、計数が黙って狂う。
...
- **フィクスチャは `cleanup().await` で明示的に解放する。** 返った時点で解放完了が保証される唯一の経路。`Drop` もプールとスキーマ名を常駐リーパー（`test_harness/reaper.rs`）へ渡すが、**これは取りこぼしのない経路ではない** — リーパーのキューに残った要求はプロセス終了時に破棄される（`reaper.rs` の `global()` が明文で述べる）。libtest は最後のテストの直後に exit するため、そのプロセスで最後に `Drop` された 1 件は完了する時間が構造的にゼロで、**バイナリごとに確実に 1 個残る**。`Drop` は同期・非ブロッキング・無謬でなければならない（アンワインド中の destructor でパニックするとプロセスが abort する）ため、実行そのものを呼び出し側のランタイムから外に出しており、この構造の帰結として避けられない。残ったスキーマは次回以降の起動時スイープが回収するので破綻はしないが、**「1 回の実行で新規残留 0」を保てるのは `cleanup()` を徹底したときだけ**である。
  - **これを自動で検出する仕組みは無い。** CI・lint とも該当するものが無く、`tests/harness_release_it.rs` は `wait_until_reclaimed` でプロセス内待機するため**プロセス終了時の取りこぼしという当のふるまいを検出できない**。規律だけが担保になっている。
