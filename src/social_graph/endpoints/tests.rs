//! Router-level tests for `endpoints.rs`'s nine handlers (Requirements 1.8,
//! 2.7, 4.6, 5.6, 10.1, 10.2, 10.4, 10.5), driven through a real, test-only
//! axum `Router` dispatched via `tower::ServiceExt::oneshot` against a real,
//! `spawn_test_app`-backed Postgres schema -- mirrors
//! `crate::oauth::middleware::tests`'s own established "real router, real
//! DB, no mocked auth" precedent, and reuses `follow_service/tests.rs` et
//! al.'s own `build_service`/`RecordingSink`/real-actor-row fixture
//! conventions for constructing the four business services this module's
//! handlers close over. No `tests/*_it.rs` integration test is added here --
//! see `endpoints.rs`'s own doc comment ("Testing strategy") for why this
//! task's own boundary stops at `#[cfg(test)] mod tests`, and this module's
//! own `build_router` builds a router that mounts these nine handlers
//! nowhere else reaches yet (task 5.2 mounts them onto the real production
//! router).
//!
...
//! guarantee correctly -- not re-testing the service's own logic, per this
...
use crate::federation::{CanonicalActivity, DeliveryService, DeliveryTarget};
...
use crate::social_graph::activity_builder::{ActivityBuilder, PgRemoteActorLookup};
...
) -> crate::accounts::model::RemoteAccount {
    crate::accounts::model::RemoteAccount {
        id,
        ...
        avatar_url: None,
        header_url: None,
        fields: Vec::<crate::accounts::model::ProfileField>::new(),
        bot: false,
        locked: false,
        fetched_at,
    }
...
async fn create_test_remote(app: &TestApp, actor_uri: &str, username: &str) -> Id {
    ...
    let now = app.runtime.clock.now();
    crate::accounts::remote_repository::upsert_remote(
        &app.pool,
...
impl DeliverySink for Arc<NoopSink> {
    async fn dispatch(
        &self,
...
fn build_state(app: &TestApp) -> TestState {
    let local_sink = Arc::new(NoopSink::new());
    let http_sink = Arc::new(NoopSink::new());
    let delivery = Arc::new(DeliveryService::new(
        RecipientTargetResolver::new(ActorDirectory::new(app.pool.clone())),
        Arc::clone(&local_sink),
        Arc::clone(&http_sink),
    ));
    let ids = Arc::new(SeqIdGenerator::new(120_000)) as Arc<dyn crate::runtime::IdGenerator>;
    ...
    let build_activity_builder = || {
        ActivityBuilder::new(
            crate::federation::urls::ActorUrls::new("kawasemi.example"),
            Arc::clone(&ids),
...
async fn list_follow_requests_succeeds_with_follow_scope() {
    ...
    let req = crate::social_graph::model::FollowRequest {
        requester: crate::domain::AccountRef::Remote(requester),
        target: crate::domain::AccountRef::Local(owner),
