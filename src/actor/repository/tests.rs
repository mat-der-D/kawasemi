...
async fn insert_actor_rejects_duplicate_handle_with_a_client_error() {
    ...
    let by_id = find_by_id(&db.pool, second_id)
        .await
        .expect("find_by_id must succeed");
    assert!(
        by_id.is_none(),
    ...
    let by_handle = find_by_handle(&db.pool, &first_actor.handle)
        .await
        .expect("find_by_handle must succeed")
...
async fn list_by_owner_returns_only_that_owners_actors() {
    ...
    create_owner(&db.pool, owner_a, now)
        .await
        .expect("creating owner A must succeed");
    create_owner(&db.pool, owner_b, now)
        .await
        .expect("creating owner B must succeed");

    let a1 = sample_actor(owner_a, db.runtime.ids.next_id(), "a_one", now);
    let a2 = sample_actor(owner_a, db.runtime.ids.next_id(), "a_two", now);
    let b1 = sample_actor(owner_b, db.runtime.ids.next_id(), "b_one", now);
    ...
    let mut owner_a_actors = list_by_owner(&db.pool, owner_a)
        .await
    ...
    let owner_b_actors = list_by_owner(&db.pool, owner_b)
        .await
...
async fn list_by_owner_returns_empty_for_an_owner_with_no_actors() {
    ...
    let actors = list_by_owner(&db.pool, owner_id)
        .await
...
async fn update_state_transitions_state_and_updated_at_and_reports_true() {
    ...
    let updated = update_state(&db.pool, actor_id, ActorState::Deactivated, later)
        .await
    ...
    let found = find_by_id(&db.pool, actor_id)
        .await
        .expect("find_by_id must succeed")
...
async fn update_state_returns_false_for_an_unknown_id() {
    ...
    let now = db.runtime.clock.now();
    let updated = update_state(&db.pool, unknown_id, ActorState::Deactivated, now)
        .await
...
async fn find_by_handle_returns_none_for_an_unknown_handle() {
    ...
    let found = find_by_handle(&db.pool, &unknown_handle)
        .await
