...
//! there is no reachability-skip preflight here: `spawn_test_db` itself
...
async fn create_owner_persists_a_row_that_find_owner_returns_unchanged() {
    ...
    let created = create_owner(&db.pool, id, now)
        .await
    ...
    let found = find_owner(&db.pool, id)
        .await
...
async fn create_owner_persists_distinct_rows_for_distinct_ids() {
    ...
    let first = create_owner(&db.pool, first_id, now)
        .await
        .expect("creating the first owner must succeed");
    let second = create_owner(&db.pool, second_id, now)
        .await
        .expect("creating the second owner must succeed");
    assert_ne!(first.id, second.id);

    let found_first = find_owner(&db.pool, first_id)
        .await
        .expect("find_owner must succeed")
        .expect("the first owner must be found");
    let found_second = find_owner(&db.pool, second_id)
        .await
        .expect("find_owner must succeed")
...
async fn find_owner_returns_none_for_an_unknown_id() {
    ...
    let found = find_owner(&db.pool, unknown_id)
        .await
