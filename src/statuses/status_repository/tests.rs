...
//! deterministic `RuntimeContext` (`db.runtime.ids`/`db.runtime.clock`),
...
fn sample_status(
    ...
    Status {
        ...
        in_reply_to_id,
        in_reply_to_account_id: in_reply_to_id.map(|_| db.runtime.ids.next_id()),
        reblog_of_id,
        poll_id: None,
...
async fn insert(db: &TestDb, status: &Status) {
...
async fn insert_status_persists_a_row_findable_by_id() {
    ...
    let status = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &status).await;

    let found = find_visible(&db.pool, status.id, None)
        .await
...
async fn insert_status_rejects_duplicate_uri_with_a_client_error() {
    let db = spawn_test_db().await;
    let actor_id = db.runtime.ids.next_id();
    let first = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &first).await;

    let mut second = sample_status(&db, actor_id, Visibility::Public, None, None);
    second.uri = first.uri.clone();
    let err = insert_status(&db.pool, &second)
        .await
...
async fn find_visible_returns_none_for_an_unknown_id() {
    ...
    let found = find_visible(&db.pool, unknown, None)
        .await
...
async fn find_visible_shows_private_status_to_its_author_but_hides_it_from_others() {
    ...
    let other = db.runtime.ids.next_id();
    let status = sample_status(&db, author, Visibility::Private, None, None);
    insert(&db, &status).await;

    let as_author = find_visible(&db.pool, status.id, Some(author))
        .await
        .expect("find_visible must succeed");
    assert_eq!(as_author, Some(status.clone()));

    let as_other = find_visible(&db.pool, status.id, Some(other))
        .await
        .expect("find_visible must succeed");
...
async fn find_visible_shows_direct_status_to_its_author_but_hides_it_from_others() {
    ...
    let author = db.runtime.ids.next_id();
    let other = db.runtime.ids.next_id();
    let status = sample_status(&db, author, Visibility::Direct, None, None);
    insert(&db, &status).await;

    assert!(
        find_visible(&db.pool, status.id, Some(author))
            .await
            .unwrap()
            .is_some()
    );
    assert!(
        find_visible(&db.pool, status.id, Some(other))
            .await
            .unwrap()
            .is_none()
    );
...
async fn find_visible_unauthenticated_viewer_sees_only_public_and_unlisted() {
    let db = spawn_test_db().await;
    let author = db.runtime.ids.next_id();
    let public = sample_status(&db, author, Visibility::Public, None, None);
    let unlisted = sample_status(&db, author, Visibility::Unlisted, None, None);
    let private = sample_status(&db, author, Visibility::Private, None, None);
    let direct = sample_status(&db, author, Visibility::Direct, None, None);
    for status in [&public, &unlisted, &private, &direct] {
        insert(&db, status).await;
    }

    assert!(
        find_visible(&db.pool, public.id, None)
            .await
            .unwrap()
            .is_some()
    );
    assert!(
        find_visible(&db.pool, unlisted.id, None)
            .await
            .unwrap()
            .is_some()
    );
    assert!(
        find_visible(&db.pool, private.id, None)
            .await
            .unwrap()
            .is_none()
    );
    assert!(
        find_visible(&db.pool, direct.id, None)
            .await
            .unwrap()
            .is_none()
    );
...
async fn ancestors_returns_the_reply_chain_oldest_first() {
    ...
    let root = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &root).await;
    let mid = sample_status(&db, actor_id, Visibility::Public, Some(root.id), None);
    insert(&db, &mid).await;
    let leaf = sample_status(&db, actor_id, Visibility::Public, Some(mid.id), None);
    insert(&db, &leaf).await;

    let chain = ancestors(&db.pool, leaf.id, None)
        .await
...
async fn ancestors_excludes_invisible_nodes_but_keeps_walking_past_them() {
    ...
    let root = sample_status(&db, author, Visibility::Public, None, None);
    insert(&db, &root).await;
    // A private reply the viewer cannot see, sandwiched between root and leaf.
    let hidden_mid = sample_status(&db, author, Visibility::Private, Some(root.id), None);
    insert(&db, &hidden_mid).await;
    let leaf = sample_status(&db, author, Visibility::Public, Some(hidden_mid.id), None);
    insert(&db, &leaf).await;

    let chain = ancestors(&db.pool, leaf.id, Some(viewer))
        .await
        .expect("ancestors must succeed");
    assert_eq!(
        chain.iter().map(|s| s.id).collect::<Vec<_>>(),
...
async fn ancestors_returns_empty_for_a_non_reply_status() {
    ...
    let chain = ancestors(&db.pool, status.id, None)
        .await
        .expect("ancestors must succeed");
...
async fn descendants_returns_the_full_reply_tree_chronologically() {
    ...
    let root = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &root).await;
    let child_a = sample_status(&db, actor_id, Visibility::Public, Some(root.id), None);
    insert(&db, &child_a).await;
    let child_b = sample_status(&db, actor_id, Visibility::Public, Some(root.id), None);
    insert(&db, &child_b).await;
    let grandchild = sample_status(&db, actor_id, Visibility::Public, Some(child_a.id), None);
    insert(&db, &grandchild).await;

    let tree = descendants(&db.pool, root.id, None)
        .await
...
async fn descendants_excludes_invisible_nodes_but_keeps_walking_past_them() {
    ...
    let root = sample_status(&db, author, Visibility::Public, None, None);
    insert(&db, &root).await;
    let hidden_child = sample_status(&db, author, Visibility::Direct, Some(root.id), None);
    insert(&db, &hidden_child).await;
    let visible_grandchild =
        sample_status(&db, author, Visibility::Public, Some(hidden_child.id), None);
    insert(&db, &visible_grandchild).await;

    let tree = descendants(&db.pool, root.id, Some(viewer))
        .await
        .expect("descendants must succeed");
    assert_eq!(
...
async fn delete_status_cascades_boosts_and_decrements_parent_replies_count() {
    ...
    let parent = sample_status(&db, author, Visibility::Public, None, None);
    insert(&db, &parent).await;
    let reply = sample_status(&db, author, Visibility::Public, Some(parent.id), None);
    insert(&db, &reply).await;
    ...
    adjust_counts(&db.pool, parent.id, CountKind::Replies, 1)
        .await
        .expect("adjust_counts must succeed");

    let boost = sample_status(&db, booster, Visibility::Public, None, Some(reply.id));
    insert(&db, &boost).await;

    delete_status(&db.pool, reply.id)
        .await
        .expect("delete_status must succeed");

    // The deleted status itself is gone.
    assert!(
        find_visible(&db.pool, reply.id, Some(author))
            .await
            .unwrap()
            .is_none()
    );
    ...
    assert!(
        find_visible(&db.pool, boost.id, Some(booster))
            .await
            .unwrap()
            .is_none(),
    ...
    let parent_after = find_visible(&db.pool, parent.id, None)
        .await
        .unwrap()
...
async fn delete_status_cascades_status_edits_via_physical_fk() {
    ...
    let now = db.runtime.clock.now() + Duration::seconds(1);
    apply_edit(&db.pool, status.id, &edit, now)
        .await
        .expect("apply_edit must succeed");
    assert_eq!(
        list_edits(&db.pool, status.id).await.unwrap().len(),
        1,
        "sanity: the history row exists before deletion"
    );

    delete_status(&db.pool, status.id)
        .await
        .expect("delete_status must succeed");

    let remaining = list_edits(&db.pool, status.id)
        .await
...
async fn delete_status_serializes_concurrent_deletes_of_the_same_reply() {
    ...
    adjust_counts(&db.pool, parent.id, CountKind::Replies, 2)
        .await
        .expect("adjust_counts must succeed");
...
async fn apply_edit_updates_live_row_and_archives_the_pre_edit_content() {
    ...
    let now = db.runtime.clock.now() + Duration::seconds(30);
    apply_edit(&db.pool, status.id, &edit, now)
        .await
        .expect("apply_edit must succeed");

    let updated = find_visible(&db.pool, status.id, None)
        .await
        .unwrap()
    ...
    let history = list_edits(&db.pool, status.id)
        .await
...
async fn apply_edit_second_edit_archives_the_first_edited_version() {
    ...
    let t1 = db.runtime.clock.now() + Duration::seconds(10);
    apply_edit(&db.pool, status.id, &first_edit, t1)
        .await
    ...
    apply_edit(&db.pool, status.id, &second_edit, t2)
        .await
    ...
    let history = list_edits(&db.pool, status.id).await.unwrap();
...
async fn apply_edit_returns_a_client_not_found_error_for_an_unknown_id() {
    ...
    let err = apply_edit(&db.pool, unknown, &edit, db.runtime.clock.now())
        .await
...
async fn adjust_counts_increments_and_decrements_the_targeted_column_only() {
    ...
    adjust_counts(&db.pool, status.id, CountKind::Reblogs, 3)
        .await
        .expect("adjust_counts must succeed");
    adjust_counts(&db.pool, status.id, CountKind::Favourites, 5)
        .await
        .expect("adjust_counts must succeed");
    adjust_counts(&db.pool, status.id, CountKind::Reblogs, -1)
        .await
        .expect("adjust_counts must succeed");
...
async fn adjust_counts_floors_at_zero() {
    ...
    adjust_counts(&db.pool, status.id, CountKind::Favourites, -10)
        .await
        .expect("adjust_counts must succeed");
...
async fn list_by_actor_returns_only_that_actors_statuses_newest_first() {
    let db = spawn_test_db().await;
    let actor_id = db.runtime.ids.next_id();
    let other_actor_id = db.runtime.ids.next_id();

    let first = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &first).await;
    let second = sample_status(&db, actor_id, Visibility::Private, None, None);
    insert(&db, &second).await;
    let other = sample_status(&db, other_actor_id, Visibility::Public, None, None);
    insert(&db, &other).await;

    let listed = list_by_actor(&db.pool, actor_id)
        .await
...
async fn list_by_actor_applies_no_filtering_of_its_own() {
    let db = spawn_test_db().await;
    let actor_id = db.runtime.ids.next_id();
    let parent = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &parent).await;
    let reply = sample_status(&db, actor_id, Visibility::Public, Some(parent.id), None);
    insert(&db, &reply).await;
    let reblog = sample_status(&db, actor_id, Visibility::Public, None, Some(parent.id));
    insert(&db, &reblog).await;
    let direct = sample_status(&db, actor_id, Visibility::Direct, None, None);
    insert(&db, &direct).await;
...
async fn count_for_actor_counts_only_that_actors_own_statuses() {
    ...
    assert_eq!(
        count_for_actor(&db.pool, actor_id).await.unwrap(),
        0,
    ...
    insert(
        &db,
        &sample_status(&db, actor_id, Visibility::Public, None, None),
    )
    .await;
    insert(
        &db,
        &sample_status(&db, actor_id, Visibility::Private, None, None),
    )
    .await;
    insert(
        &db,
        &sample_status(&db, other_actor_id, Visibility::Public, None, None),
    )
    .await;

    assert_eq!(count_for_actor(&db.pool, actor_id).await.unwrap(), 2);
    assert_eq!(count_for_actor(&db.pool, other_actor_id).await.unwrap(), 1);
...
async fn last_created_at_for_actor_reflects_the_most_recent_post() {
    ...
    assert_eq!(
        last_created_at_for_actor(&db.pool, actor_id).await.unwrap(),
        None,
        "an actor with no posts has no last_status_at"
    );

    let mut first = sample_status(&db, actor_id, Visibility::Public, None, None);
    first.created_at -= Duration::seconds(60);
    insert(&db, &first).await;

    let second = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &second).await;

    let last = last_created_at_for_actor(&db.pool, actor_id)
        .await
        .unwrap()
...
async fn media_ids_for_statuses_matches_calling_the_singular_version_per_status() {
    ...
    let with_many = sample_status(&db, actor_id, Visibility::Public, None, None);
    let with_one = sample_status(&db, actor_id, Visibility::Public, None, None);
    let without_media = sample_status(&db, actor_id, Visibility::Public, None, None);
    insert(&db, &with_many).await;
    insert(&db, &with_one).await;
    insert(&db, &without_media).await;
    ...
    attach_media(&db.pool, with_many.id, &[media_a, media_b, media_c])
        .await
        .expect("attach_media must succeed");
    attach_media(&db.pool, with_one.id, &[media_b])
        .await
        .expect("attach_media must succeed");
    ...
    for &status_id in &ids {
        let singular = media_ids_for_status(&db.pool, status_id)
            .await
    ...
    let batched = media_ids_for_statuses(&db.pool, &ids)
        .await
...
async fn media_ids_for_statuses_returns_empty_for_an_empty_slice_without_querying() {
    ...
    let batched = media_ids_for_statuses(&db.pool, &[])
        .await
...
async fn write_functions_accept_a_transaction_and_roll_back_together() {
    ...
    let child = sample_status(&db, actor_id, Visibility::Public, Some(parent.id), None);
    let media_ids = vec![db.runtime.ids.next_id(), db.runtime.ids.next_id()];
    ...
    let found = find_visible(&db.pool, child.id, None)
        .await
        .expect("find_visible must succeed");
    assert!(found.is_none(), "a rolled-back insert must leave no row");

    let attached = media_ids_for_status(&db.pool, child.id)
        .await
        .expect("media_ids_for_status must succeed");
    assert!(
        attached.is_empty(),
...
async fn write_functions_committed_through_a_transaction_persist_normally() {
    ...
    let child = sample_status(&db, actor_id, Visibility::Public, Some(parent.id), None);
    let media_a = db.runtime.ids.next_id();
    let media_b = db.runtime.ids.next_id();
