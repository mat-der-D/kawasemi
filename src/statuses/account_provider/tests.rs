...
//! `AccountStatusesProviderImpl` hands the assembler.
//!
...
//! renders, and what a page costs in queries, is verified in
//! `tests/statuses_account_provider_it.rs`: those tests reach the provider's
//! live handles through `AppState` and so need a real running instance.
//! What is left here is SQL plus domain types and runs on
//! `crate::test_harness::db_fixture::spawn_test_db`.
//!
//! The row shapes mirror `statuses/poll_repository/tests.rs`'s fixture
