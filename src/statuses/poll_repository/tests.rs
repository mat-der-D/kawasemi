...
async fn insert_poll_persists_options_with_zero_initial_votes() {
    ...
    let actor = db.runtime.ids.next_id();
    let poll = insert_target_poll(&db, actor, &["Pizza", "Sushi", "Tacos"], false, None).await;

    let result = tally(&db.pool, poll.id, None)
        .await
...
async fn tally_reports_not_found_for_unknown_poll() {
    let db = spawn_test_db().await;
    let unknown_poll = db.runtime.ids.next_id();

    let err = tally(&db.pool, unknown_poll, None)
        .await
...
async fn record_vote_records_a_valid_single_choice_vote_and_updates_tally() {
    let db = spawn_test_db().await;
    let owner = db.runtime.ids.next_id();
    let voter = db.runtime.ids.next_id();
    let poll = insert_target_poll(&db, owner, &["Yes", "No"], false, None).await;

    let outcome = record_vote(&db.pool, poll.id, voter, &[0], db.runtime.clock.now())
        .await
        .expect("record_vote must succeed for a valid single-choice vote");
    assert_eq!(outcome, VoteOutcome::Recorded);

    let result = tally(&db.pool, poll.id, Some(voter))
        .await
        .expect("tally must succeed");
...
async fn record_vote_records_multiple_choices_on_a_multiple_choice_poll() {
    ...
    let owner = db.runtime.ids.next_id();
    let voter = db.runtime.ids.next_id();
    let poll = insert_target_poll(&db, owner, &["Pizza", "Sushi", "Tacos"], true, None).await;

    let outcome = record_vote(&db.pool, poll.id, voter, &[0, 2], db.runtime.clock.now())
        .await
...
async fn record_vote_accumulates_across_independent_voters() {
    let db = spawn_test_db().await;
    let owner = db.runtime.ids.next_id();
    let voter_a = db.runtime.ids.next_id();
    let voter_b = db.runtime.ids.next_id();
    let poll = insert_target_poll(&db, owner, &["Yes", "No"], false, None).await;

    record_vote(&db.pool, poll.id, voter_a, &[0], db.runtime.clock.now())
        .await
        .expect("voter_a's vote must succeed");
    record_vote(&db.pool, poll.id, voter_b, &[0], db.runtime.clock.now())
        .await
...
async fn record_vote_rejects_when_deadline_has_passed() {
    ...
    let poll = insert_target_poll(&db, owner, &["Yes", "No"], false, Some(already_expired)).await;

    let err = record_vote(&db.pool, poll.id, voter, &[0], now)
        .await
...
async fn record_vote_rejects_when_now_equals_expires_at() {
    ...
    let voter = db.runtime.ids.next_id();
    let now = db.runtime.clock.now();
    let poll = insert_target_poll(&db, owner, &["Yes", "No"], false, Some(now)).await;
...
async fn record_vote_rejects_out_of_range_choice_index() {
    ...
    let err = record_vote(&db.pool, poll.id, voter, &[7], db.runtime.clock.now())
        .await
...
async fn record_vote_rejects_negative_choice_index() {
    ...
    let err = record_vote(&db.pool, poll.id, voter, &[-1], db.runtime.clock.now())
        .await
...
async fn record_vote_rejects_multiple_choices_on_a_single_choice_poll() {
    ...
    let owner = db.runtime.ids.next_id();
    let voter = db.runtime.ids.next_id();
    let poll = insert_target_poll(&db, owner, &["Yes", "No", "Maybe"], false, None).await;

    let err = record_vote(&db.pool, poll.id, voter, &[0, 1], db.runtime.clock.now())
        .await
...
async fn record_vote_rejects_an_empty_choice_list() {
    ...
    let err = record_vote(&db.pool, poll.id, voter, &[], db.runtime.clock.now())
        .await
...
async fn record_vote_rejects_a_duplicate_vote_by_the_same_actor() {
    ...
    record_vote(&db.pool, poll.id, voter, &[0], now)
        .await
...
async fn record_vote_rejects_a_resubmission_with_a_different_choice() {
    ...
    let err = record_vote(&db.pool, poll.id, voter, &[1], now)
        .await
...
async fn record_vote_deduplicates_repeated_choice_within_one_request() {
    ...
    let owner = db.runtime.ids.next_id();
    let voter = db.runtime.ids.next_id();
    let poll = insert_target_poll(&db, owner, &["Yes", "No"], true, None).await;

    let outcome = record_vote(&db.pool, poll.id, voter, &[0, 0], db.runtime.clock.now())
        .await
...
async fn record_vote_is_scoped_to_its_own_poll() {
    ...
    let owner = db.runtime.ids.next_id();
    let voter = db.runtime.ids.next_id();
    let poll_a = insert_target_poll(&db, owner, &["Yes", "No"], false, None).await;
    let poll_b = insert_target_poll(&db, owner, &["Yes", "No"], false, None).await;

    record_vote(&db.pool, poll_a.id, voter, &[0], db.runtime.clock.now())
        .await
        .expect("vote on poll_a must succeed");
    // The same actor voting on a *different* poll is not a duplicate.
    record_vote(&db.pool, poll_b.id, voter, &[1], db.runtime.clock.now())
        .await
        .expect("vote on poll_b must succeed independently of poll_a");

    let tally_a = tally(&db.pool, poll_a.id, None).await.unwrap();
    let tally_b = tally(&db.pool, poll_b.id, None).await.unwrap();
...
async fn record_vote_reports_not_found_for_unknown_poll() {
    ...
    let err = record_vote(&db.pool, unknown_poll, voter, &[0], db.runtime.clock.now())
        .await
...
async fn batch_fixture(db: &TestDb) -> BatchFixture {
    ...
    .await;
    let single = insert_target_poll_with_options(db, owner, &[(0, "Yes"), (1, "No")], false).await;
    let quiet = insert_target_poll_with_options(db, owner, &[(0, "Yes"), (1, "No")], false).await;
    let optionless = insert_target_poll_with_options(db, owner, &[], false).await;
    ...
    record_vote(&db.pool, ordered.id, viewer, &[0, 2], now)
        .await
        .expect("viewer's multi-choice vote must succeed");
    record_vote(&db.pool, ordered.id, other_actor, &[1], now)
        .await
        .expect("other_actor's vote must succeed");
    record_vote(&db.pool, single.id, viewer, &[1], now)
        .await
        .expect("viewer's vote must succeed");
    record_vote(&db.pool, single.id, other_actor, &[0], now)
        .await
        .expect("other_actor's vote must succeed");
...
async fn tallies_per_call(db: &TestDb, ids: &[Id], viewer: Option<Id>) -> HashMap<Id, PollTally> {
    let mut per_call: HashMap<Id, PollTally> = HashMap::new();
    for &poll_id in ids {
        match tally(&db.pool, poll_id, viewer).await {
            Ok(result) => {
...
async fn find_polls_by_ids_matches_calling_the_singular_version_per_poll() {
    ...
    let open = insert_target_poll(&db, owner, &["Yes", "No"], false, None).await;
    let multiple = insert_target_poll(&db, owner, &["Pizza", "Sushi"], true, None).await;
    ...
    for &poll_id in &ids {
        let singular = find_poll_by_id(&db.pool, poll_id)
            .await
    ...
    let batched = find_polls_by_ids(&db.pool, &ids)
        .await
...
async fn tally_many_matches_calling_the_singular_version_per_poll() {
    ...
    for viewer in [Some(fixture.viewer), Some(fixture.other_actor), None] {
        let per_call = tallies_per_call(&db, &fixture.ids, viewer).await;
        let batched = tally_many(&db.pool, &fixture.ids, viewer)
            .await
    ...
    // sides were to degrade the same way.
    let mine = tally_many(&db.pool, &fixture.ids, Some(fixture.viewer))
        .await
        .expect("tally_many must succeed");
    let theirs = tally_many(&db.pool, &fixture.ids, Some(fixture.other_actor))
        .await
        .expect("tally_many must succeed");
    let anonymous = tally_many(&db.pool, &fixture.ids, None)
        .await
        .expect("tally_many must succeed");
...
/// share one closed pool rather than one fixture each: the assertion is
/// identical and a fixture is the scarce
...
async fn batched_poll_reads_return_empty_for_an_empty_slice_without_querying() {
    ...
    assert!(
        find_polls_by_ids(&db.pool, &[])
            .await
            .expect("an empty slice must succeed even against a closed pool")
            .is_empty()
    );
    assert!(
        tally_many(&db.pool, &[], Some(viewer))
            .await
            .expect("an empty slice must succeed even against a closed pool")
            .is_empty()
    );
    assert!(
        tally_many(&db.pool, &[], None)
            .await
            .expect("an empty slice must succeed even against a closed pool")
            .is_empty()
    );
...
async fn insert_poll_accepts_a_transaction_and_rolls_back() {
    ...
    let found = find_poll_by_id(&db.pool, poll.id)
        .await
        .expect("find_poll_by_id must succeed");
    assert!(
        found.is_none(),
...
async fn insert_poll_committed_through_a_transaction_persists_normally() {
    ...
    let tallied = tally(&db.pool, poll.id, None)
        .await
        .expect("tally must succeed");
